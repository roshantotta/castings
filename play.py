...
class CameraManager:
    ...
    def enumerate_devices():
        """Returns dictionary of {serial: device_info} for all connected cameras"""
    ...
    def auto_assign_cameras():
        """Automatically assign detected cameras to cam1, cam2, cam3 based on serial number order"""
        devices = CameraManager.enumerate_devices()
        serials = sorted(devices.keys())  # Sort serials for consistent ordering
        ...
        if len(serials) >= 3:
            ...
            return {f'cam{i+1}': serial for i, serial in enumerate(serials)}
        return {}
    ...
    def initialize_all_cameras():
        """Initialize all cameras with automatic assignment"""
        global config
        
        # Get or create camera assignments
        if 'camera_serials' not in config:
            config['camera_serials'] = CameraManager.auto_assign_cameras()
            with open("config.yaml", "w") as f:
                yaml.dump(config, f)
        
        initialized_cameras = {}
        
        for cam_name, serial in config['camera_serials'].items():
            try:
                cam = CameraManager.initialize_camera(serial)
                initialized_cameras[cam_name] = cam
                logger.info(f"Initialized {cam_name} with serial {serial}")
            except Exception as e:
                logger.error(f"Failed to initialize {cam_name} (serial {serial}): {e}")
...
def generate_frames(cam_label):
    ...
    with stream_lock:
        ...
        if not cam:
            logger.error(f"No camera instance found for {cam_label}")
            return
...
def stream():
    ...
    return render_template('stream.html', cameras=config.get('camera_serials', {}).keys())
...
@login_required
def start_stream():
    global streaming_active, camera_instances
    streaming_active = True
    camera_instances = CameraManager.initialize_all_cameras()
    return '', 204
...
# App Initialization
def init_app():
    global db_manager, camera_instances
    ...
    # Automatically assign camera serial numbers if not already configured
    if 'camera_serials' not in config:
        config['camera_serials'] = CameraManager.auto_assign_cameras()
        with open("config.yaml", "w") as f:
            yaml.dump(config, f)
